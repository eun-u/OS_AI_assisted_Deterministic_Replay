...
"$ROOT/build/adr" run --target "$ROOT/build/tests/drift_demo" \
  --trace-root "$ROOT/traces" --report "$ROOT/reports/drift.md" --llm-analysis
...
"$ROOT/build/adr" run --target "$ROOT/scripts/deadlock_timeout.sh" \
  --trace-root "$ROOT/traces" --report "$ROOT/reports/deadlock_timeout.md" --llm-analysis
