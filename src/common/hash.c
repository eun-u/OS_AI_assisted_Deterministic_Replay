...
static void adr_sha256_transform(adr_sha256_ctx *ctx, const uint8_t data[]) {
...
void adr_sha256_update(adr_sha256_ctx *ctx, const uint8_t *data, size_t len) {
  for (size_t i = 0; i < len; ++i) {
    ...
    if (ctx->datalen == 64) {
      adr_sha256_transform(ctx, ctx->data);
      ctx->bitlen += 512;
      ctx->datalen = 0;
    }
  }
...
void adr_sha256_bytes_hex(const void *data, size_t len, char out[ADR_SHA256_HEX_LEN]) {
  ...
  adr_sha256_ctx ctx;
  adr_sha256_init(&ctx);
  adr_sha256_update(&ctx, (const uint8_t *)data, len);
  adr_sha256_final(&ctx, hash);
  adr_sha256_hex(hash, out);
}
